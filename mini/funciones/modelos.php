...
abstract class modelo
{
  ...
  //-------------------------------------------------------------------------
  //-------------------------------------------------------------------------
  //Metodos para gestionar mensajes de error del modelo y facilitar el uso
  //de los mismos en la visualización final o en el control de ejecución.
  //-------------------------------------------------------------------------
  //-------------------------------------------------------------------------
  
  //-------------------------------------------------------------------------
  //Atributo con la lista de mensajes de error del modelo.
  //Es un array que contendrá elementos de tipo [ "clave" => array( mensaje, 
  //mensaje, ...), ... ], donde la clave será el nombre de un atributo o la
  //palabra que se desee utilizar, y el valor será un array con la lista de 
  //mensajes de error añadidos.
  protected $errores= array();
  
  //-------------------------------------------------------------------------
  //Saber si el modelo contiene errores.
  ...
  //-------------------------------------------------------------------------
  //Saber si un atributo concreto contiene errores.
  public function hayErrorEn( $atributo)
  {
    if (empty($atributo)) $atributo= '_';
    $res= false;
    if (isset($this->errores[$atributo])) $res= (count( $this->errores[$atributo]) > 0);
    return $res;
  }//hayErrorEn
  
  //-------------------------------------------------------------------------
  //Limpiar los errores del modelo.
  ...
  //-------------------------------------------------------------------------
  //Obtener la lista de errores del modelo.
  //Se devuelve una copia del array de error almacenados en el objeto.
  ...
  //-------------------------------------------------------------------------
  //Obtener la lista de errores de un atributo.
  //Se devuelve el array de cadenas de error o "null" si no tiene ningún
  //error el atributo indicado.
  public function obtenerErrores( $atributo)
  {
    $res= null;
    if (empty($atributo)) $atributo= '_';
    if (isset($this->errores[$atributo])) $res= $this->errores[$atributo];
    return $res;
  }//obtenerErrores
  
  //-------------------------------------------------------------------------
  //Obtener el primer error de la lista de errores de un atributo.
  //Se devuelve la cadena con el primer error de la lista o "null" si no 
  //tiene ningún error el atributo indicado.
  public function obtenerError( $atributo)
  {
    $res= null;
    if (empty($atributo)) $atributo= '_';
    if (isset($this->errores[$atributo])) $res= $this->errores[$atributo][0];
    return $res;
  }//obtenerError
  
  //-------------------------------------------------------------------------
  //Agregar un error a la lista de errores del modelo.
  //Se debe indicar la cadena con el mensaje a incluir y el nombre del 
  //atributo al que añadir el error para que queden clasificados.
  public function ponerError( $atributo, $mensaje)
  {
    if (empty($atributo)) $atributo= '_';
    if (!isset($this->errores[$atributo])) $this->errores[$atributo]= array();
    $this->errores[$atributo][]= $mensaje;
  }//ponerError
